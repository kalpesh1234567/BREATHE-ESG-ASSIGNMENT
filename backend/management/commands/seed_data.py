...
SAP_CSV = b"""Materialbel.;Pos.;Buchungsdatum;Bewegungsart;Werk;Lagerort;Materialnummer;Materialbezeichnung;Menge;ME;Betrag HW;Währg;Kostenstelle;Auftrag
...
5000012347;0001;08.01.2024;261;1000;0002;HZOEL-001;Heizöl EL;-1.200,000;L;-1.404,00;EUR;KOST-2200;
...
5000012352;0001;25.01.2024;261;2000;0003;HZOEL-001;Heizöl EL;-890,000;L;-1.041,30;EUR;KOST-4200;
...
5000012356;0001;14.02.2024;261;1000;0002;HZOEL-001;Heizöl EL;-1.100,000;L;-1.287,00;EUR;KOST-2200;
...
5000012361;0001;15.03.2024;261;1000;0002;HZOEL-001;Heizöl EL;-950,000;L;-1.111,50;EUR;KOST-2200;
...
UTILITY_CSV = b"""Account_Number,Meter_ID,Service_Address,Billing_Start,Billing_End,Usage (kWh),Demand (kW),Rate_Schedule,Total_Bill_USD
...
TRAVEL_CSV = b"""Report_ID,Report_Name,Employee_ID,Employee_Name,Department,Expense_Type,Transaction_Date,Vendor,Amount,Currency,Origin,Destination,Cabin_Class,Nights,Description
    ...
    def handle(self, *args, **kwargs):
        ...
        factors = [
            ...
            dict(category='fuel_heating_oil', value='2.54', unit='per_liter',
                 source='DEFRA 2024', region='Global', valid_from=date(2024, 1, 1),
                 notes='Light fuel oil / Heizöl EL. kgCO2e per liter.'),
            ...
            # Scope 3 — Air travel (DEFRA 2024, includes 1.7× Radiative Forcing uplift)
            # RF uplift is DEFRA recommended for Scope 3 reporting to capture non-CO2 climate effects
            dict(category='flight_economy_domestic', value='0.24531', unit='per_pkm',
                 source='DEFRA 2024', region='Global', valid_from=date(2024, 1, 1),
                 notes='Economy class, domestic (<500km). Includes 1.7× RF uplift.'),
            dict(category='flight_economy_shorthaul', value='0.12576', unit='per_pkm',
                 source='DEFRA 2024', region='Global', valid_from=date(2024, 1, 1),
                 notes='Economy class, short-haul (500-3700km). Includes 1.7× RF uplift.'),
            dict(category='flight_economy_longhaul', value='0.11704', unit='per_pkm',
                 source='DEFRA 2024', region='Global', valid_from=date(2024, 1, 1),
                 notes='Economy class, long-haul (>3700km). Includes 1.7× RF uplift.'),
            dict(category='flight_business_shorthaul', value='0.18864', unit='per_pkm',
                 source='DEFRA 2024', region='Global', valid_from=date(2024, 1, 1),
                 notes='Business class, short-haul. Includes 1.7× RF uplift.'),
            dict(category='flight_business_longhaul', value='0.42954', unit='per_pkm',
                 ...
                 notes='Business class, long-haul. Higher per-pkm due to seat footprint. '
                       'Includes 1.7× RF uplift.'),
            dict(category='flight_premium_economy_longhaul', value='0.18726', unit='per_pkm',
                 source='DEFRA 2024', region='Global', valid_from=date(2024, 1, 1),
                 notes='Premium economy, long-haul. Includes 1.7× RF uplift.'),
            dict(category='flight_first_longhaul', value='0.54779', unit='per_pkm',
                 source='DEFRA 2024', region='Global', valid_from=date(2024, 1, 1),
                 notes='First class, long-haul. Highest per-pkm. Includes 1.7× RF uplift.'),
        ...
        for source_type, filename, content in sources:
            ...
            self.stdout.write(f'  Ingested {filename}: {run.parsed_count} records, {run.error_count} errors')
